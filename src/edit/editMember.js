...
edit_member = (id_member) => {
    const role = document.getElementById(`select_role_${id_member}`).value;
    if (role) {
        console.log(id_member, room__Id, role)
        fetch(`http://localhost:8080/members/${room__Id}/${id_member}/role?newRole=${role}`, {
            method: "PUT"
        })
            .then(response => {
                if (response.ok) {
...
delete_member = (id_member) => {
    fetch(`http://localhost:8080/members/${room__Id}/${id_member}`, {
        method: "DELETE"
    })
}
...
pre_add_member = async () => {
    ...
    if (!meeting_information.path) {
        console.log("Dang bat dau them path")
        createRoom()
    } else {
        console.log("Da co path: ", meeting_information.path)
    }
    //getAllEmployee
...
function add_member() {
    ...
    console.log(addData)
    fetch(`http://localhost:8080/meetings/${room__Id}/add`, {
        method: "POST",
        headers: {
