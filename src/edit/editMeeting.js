...
pre_edit_meeting = () => {
    meeting_information_pre_edit.innerHTML =
        `   <p>Tên cuộc họp: <input type="text" id="name" value="${meeting_information.name}"></p>
            ...
            <p>Thời gian kết thúc dự kiến: <input type="datetime-local" id="expectedEndTime" value="${meeting_information.expectedEndTime}"></p>
...
edit_meeting = (id) => {
    ...
    const expectedEndTime = document.getElementById("expectedEndTime").value;
    const department = document.getElementById("department").value;
    const room = document.getElementById("room").value;

    if (!name || !rememberCode || !startTime || !expectedEndTime || !department || !room) {
    ...
    fetch(`http://localhost:8080/meetings/${room__Id}`, {
        method: "PUT",
        headers: {
